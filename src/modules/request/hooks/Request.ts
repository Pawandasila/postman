...
export function useRunRequest(tab: RequestTab) {
  ...
  return useMutation({
    mutationFn: async () => {
      // If request has an ID (saved), use the run function but pass current data
      if (tab.requestId) {
        ...
        return await run(tab.requestId, requestData);
      }
        ...
        try {
          const transformedData = {
            ...
            requestRun: {
              ...
              createdAt: data.requestRun.createdAt
                ? (typeof data.requestRun.createdAt === 'string'
                  ? data.requestRun.createdAt
                  : new Date(data.requestRun.createdAt).toISOString())
                : undefined,
            },
            result: data.result
          };
          ...
          // Fallback to basic data if transformation fails
